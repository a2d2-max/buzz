...
test.describe("global agent config screenshots", () => {
  ...
  test("04-create-blocked-no-provider-no-global", async ({ page }) => {
    ...
    await expect(harness).toHaveText(`${BRAND_NAME} Agent`);
  ...
  test("create defaults follow a preferred harness saved while open", async ({
    ...
    const defaults = page.getByTestId("agent-ai-defaults-notice");
    await expect(defaults).toContainText(`${BRAND_NAME} Agent`);
    await defaults
  ...
  test("create exposes setup guidance when no harness is available", async ({
    ...
    await selectDropdownOption(
      page,
      harness,
      `${BRAND_NAME} Agent (not installed)`,
    );
    await expect(
      customSection
        .locator("p")
        .filter({ hasText: `${BRAND_NAME} Agent is not installed.` }),
    ).toContainText(
      `${BRAND_NAME} Agent is not installed. Ships with the ${BRAND_NAME} desktop app. Visit Settings > Agents to set it up.`,
    );
    await expect(page.getByTestId("persona-dialog-submit")).toBeDisabled();
  });
  ...
  test("05-create-enabled-with-global-provider", async ({ page }) => {
    ...
    await expect(
      defaultsSection.getByText(`${BRAND_NAME} Agent`, { exact: true }),
    ).toBeVisible();

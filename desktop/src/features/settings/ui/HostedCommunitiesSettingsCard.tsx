...
export function HostedCommunitiesSettingsCard() {
    ...
    if (
      ...
      throw new Error(
        errorMessage(
          ...
          `Could not load the connected ${BRAND_NAME} identity.`,
        ),
      );
    }
  ...
  const connectIdentity = () =>
    run(`Connecting ${BRAND_NAME} identity…`, async () => {
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            ...
            `Could not connect the ${BRAND_NAME} identity.`,
          ),
        );
      }
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            response.error,
            response.correlation_id,
            `Could not unpair the ${BRAND_NAME} identity.`,
          ),
        );
      }
      setIdentity(null);
      ...
      if (released.error) {
        throw new Error(
          errorMessage(
            ...
            `Could not release the previously connected ${BRAND_NAME} identity.`,
          ),
        );
      }
      ...
      if (bound.error) {
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ? `This device's ${BRAND_NAME} identity is already reserved by another Builderlab account, so it can't be connected here. Sign in with that account, or transfer the identity there first.`
            : errorMessage(
                bound.error,
                bound.correlation_id,
                `Could not connect this device's ${BRAND_NAME} identity.`,
              ),
        );
      }
      ...
      if (availabilityResponse.error || !availabilityResponse.available) {
        ...
        throw new Error(
          errorMessage(
            ...
            `That ${BRAND_NAME} address is already taken.`,
          ),
        );
      }
      const response = await invoke<CommunityMutationResponse>(
  ...
  return (
    <section className="space-y-6" data-testid="hosted-communities-settings">
      <SettingsSectionHeader
        ...
        description={`${BRAND_NAME} works with any relay. This page is only for relay hosting provided by Block — sign in with a Builderlab account to create and manage Block-hosted communities. Builderlab sign-in is used on this page alone.`}
      />
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border/70 p-5">
          ...
          >
            ...
            {BRAND_NAME}. You can use every other part of the app without
            signing in.
          </p>
          <Button
            className="mt-4"
          ...
          {!identity ? (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-5">
              <h3 className="font-medium">
                Link this account to your {BRAND_NAME} identity
              </h3>
              <p
                className="mt-2 text-sm text-muted-foreground/70"
                data-settings-subcopy
              >
                This Builderlab account isn&apos;t linked to an {BRAND_NAME}{" "}
                identity yet. Connect this device&apos;s key to create and own
                communities under it — {BRAND_NAME} signs a one-time challenge
              ...
              >
                ...
                {action ?? `Connect ${BRAND_NAME} identity`}
              </Button>
            </div>
          ) : identityMismatch ? (
            <div className="rounded-xl border border-amber-500/50 bg-amber-500/5 p-5">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <h3 className="font-medium">
                    This account is connected to a different {BRAND_NAME}{" "}
                    identity
                  </h3>
                  <p
                    className="mt-2 text-sm text-muted-foreground/70"
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Unpair this {BRAND_NAME} identity?
          </AlertDialogTitle>
          <AlertDialogDescription>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            member. The recipient needs a connected {BRAND_NAME} identity first,

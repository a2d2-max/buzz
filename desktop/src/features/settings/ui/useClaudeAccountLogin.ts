/**
 * In-app browser login for one Claude config-directory account: start it, follow it
...
export function claudeAccountLoginQueryKey(accountId: string) {
  return ["claudeAccountLogin", accountId] as const;
}
...
export function useClaudeAccountLogin(
  ...
  const queryKey = claudeAccountLoginQueryKey(accountId);
  ...
  const session = useQuery({
    ...
    queryKey,
    queryFn: async () => {
      const polled = await pollClaudeAccountLogin(accountId);
      const current =
  ...
  const start = useMutation({
    mutationFn: async () => {
      ...
      return startClaudeAccountLogin(accountId);
    },
    onSuccess: (snapshot) => {
  ...
  const cancel = useMutation({
    mutationFn: async () => {
      await queryClient.cancelQueries({ queryKey });
      return cancelClaudeAccountLogin(accountId);
    },
    onSuccess: (snapshot) => {
      if (snapshot) {
        queryClient.setQueryData<CodexLoginSession | null>(queryKey, snapshot);
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      // Readiness of agents on this Claude account changed.

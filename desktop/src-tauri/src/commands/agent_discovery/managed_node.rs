...
fn managed_node_unsupported_step() -> InstallStepResult {
    InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "a2d2 does not provide a managed Node.js runtime for {}-{} yet",
            std::env::consts::OS,
            std::env::consts::ARCH
        ),
        exit_code: None,
        hint: Some(
            "Install Node.js from https://nodejs.org, restart a2d2, then click Install again."
                .to_string(),
        ),
    }
}
...
fn managed_node_install_hint() -> String {
    "a2d2 could not install its private Node.js runtime. Check your network and app-data directory permissions, then click Install again.".to_string()
}
...
fn managed_npm_prefix_hint() -> String {
    "a2d2 could not create its private Node tools directory. Check app-data directory permissions, restart a2d2, then click Install again.".to_string()
}
...
pub(super) fn npm_eacces_hint(stderr: &str, _command: &str) -> Option<String> {
    if stderr.contains("EACCES: permission denied") || stderr.contains("npm error EACCES") {
        Some(
            "npm could not write to a2d2's private Node tools directory. Check app-data directory permissions, restart a2d2, then click Install again."
                .to_string(),
        )
    } else {
        None

...
const RELAY_MESH_RUNTIME_NO_TARGET: &str =
    "a2d2 shared compute requires a live serving member; start serving the selected model on a member, then try again";
...
fn restarting_share_status(config: &MeshSharingConfig) -> mesh_llm::MeshNodeStatus {
    mesh_llm::MeshNodeStatus {
        ...
        health: mesh_llm::MeshHealth {
            ...
            reason: Some("a2d2 is restarting to switch this machine to sharing".to_string()),
        },
        api_base_url: None,
        console_url: None,
...
pub async fn mesh_start_node(
    ...
    let status = match started.status().await {
        ...
        Err(error) => {
            ...
            app.request_restart();
            return Err(format!(
                "mesh node started but status probe failed: {error:#}; a2d2 is restarting to guarantee cleanup"
            ));
        }
    };
...
pub(crate) async fn ensure_relay_mesh_for_record(
    ...
    if state.mesh_llm_runtime.lock().await.is_some() {
        ...
        .await
        {
            ...
            mesh_llm::MeshRuntimeRecovery::Debouncing => {
                return Err(
                    "a2d2 shared compute ingress is temporarily unresponsive; recovery is already scheduled. Try again shortly."
                        .to_string(),
                );
            }
            mesh_llm::MeshRuntimeRecovery::ReleasePending => {
                return Err(
                    "a2d2 shared compute is still shutting down its previous local ingress. Try again shortly."
                        .to_string(),
                );
            }
            ...
            mesh_llm::MeshRuntimeRecovery::RestartRequired => {
                app.request_restart();
                return Err(
                    "a2d2 shared compute startup lost its local ingress before shutdown control became available. a2d2 is restarting to recover it."
                        .to_string(),
                );
            }
        }
    ...
    let target = match resolve_mesh_bootstrap_target(&state, model_id).await {
        ...
        Ok(None) => {
            return Err(
                "a2d2 shared compute cannot start because no live member is serving this model. Start serving it on a member, then try again."
                    .to_string(),
            );
        }
        Err(error) => {
            return Err(format!(
                "could not refresh a2d2 shared compute serving members: {error}"
            ));
        }
    };

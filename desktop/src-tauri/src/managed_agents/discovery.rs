...
pub fn missing_command_message(command: &str, role: &str) -> String {
    ...
    format!(
        "{role} `{command}` was not found. Make sure it is installed and on your PATH. Antivirus software can quarantine bundled binaries — if that happened, restore the file or reinstall a2d2. (Source builds: see TESTING.md.)"
    )
}

...
| **Agent mention claims (kind:24250)** | ✅ | Ephemeral; one `e` tag naming the mention, optional `nonce` tag (≤64 chars, **must be a per-process random value of ≥16 bytes** — never a hostname, agent name, or other constant: two runtimes sharing a nonce both win and both answer). Never stored, never fanned out — the relay answers OK only. Winner decided by an atomic Redis `SET NX EX` on `buzz:{community}:claim:{pubkey}:{target}`; loser gets `OK false "duplicate: already claimed"`, the holder re-claiming with its own nonce gets `OK true "already yours"` and its lease is refreshed to the full TTL (one Lua compare-and-expire, so nobody can win between the check and the refresh). TTL from `BUZZ_CLAIM_TTL_SECS` (default 600s). Accepted over WS and `POST /events`. |

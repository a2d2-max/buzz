...
-- ── Replaceable engine projections ──────────────────────────────────────────
-- Canonical signed A2D2 events remain authoritative. These rows contain only
-- non-secret binding metadata, principal mappings, and a retryable projection
-- head. `api_key_env` names a server-side secret source; it is not a secret.
